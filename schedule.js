...
function calculate() {
  ...
  currentTotalRepaid = totalRepaid;
...
// Correctly quotes/escapes a single CSV field per RFC 4180 — doubles any
// internal quote characters and wraps every field in quotes, which also
// safely handles values containing commas (e.g. "£250,000").
...
function downloadCSV() {
  ...
  // Summary block — every figure here is read from state already computed
  // and displayed on the page by calculate(); nothing is recalculated
  // independently, so the export can never drift from what's shown on screen.
  rows.push(csvRow(['24/7 UK Mortgage Calculator']));
  rows.push(csvRow(['Mortgage Repayment Schedule']));
  rows.push(csvRow(['Property Value', gbp.format(currentPrice)]));
  rows.push(csvRow(['Deposit', gbp.format(currentDeposit)]));
  rows.push(csvRow(['Mortgage Amount', gbp.format(currentLoan)]));
  rows.push(csvRow(['Interest Rate', `${currentRate}%`]));
  rows.push(csvRow(['Mortgage Term', `${currentYears} years`]));
  rows.push(csvRow(['Monthly Repayment', gbpDecimal.format(currentMonthlyPayment)]));
  rows.push(csvRow(['Total Repaid', gbp.format(currentTotalRepaid)]));
  rows.push(csvRow(['Total Interest', gbp.format(currentTotalInterest)]));
  rows.push(csvRow(['Loan-to-Value (LTV)', `${currentLtv.toFixed(0)}%`]));
  rows.push(''); // single blank row separating summary from the schedule

  // Monthly schedule — the same currentSchedule array already powering
  // the on-page yearly/monthly tables, just reformatted for export.
  rows.push(csvRow(['Month', 'Year', 'Payment', 'Capital Repaid', 'Interest Paid', 'Remaining Balance']));
  ...
  // Leading UTF-8 BOM + CRLF line endings for reliable Excel compatibility;
  // Google Sheets handles both correctly regardless.
  const csvContent = '\uFEFF' + rows.join('\r\n');
  const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  a.download = `mortgage-repayment-schedule-${currentYears}yr.csv`;
